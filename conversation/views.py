...
@login_required
def inbox(request):
    converstions = Conversation.objects.filter(members__in=[request.user.id])
...
def detail(request,pk):
    converstion = Conversation.objects.filter(members__in=[request.user.id]).get(pk=pk)
    ...
    return render(request,'conversation/detail.html',{'conversation':converstion,'form':form})
